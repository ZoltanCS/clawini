...
function CodeBlock({ code, language }: { code: string; language?: string }) {
  ...
  const handleDownload = () => {
    const ext = language === 'html' ? 'html' : language || 'txt';
  ...
  return (
    <>
      <div style={{ margin: '12px 0', borderRadius: '16px', overflow: 'hidden', border: '1px solid var(--border-subtle)' }}>
        <div className="flex items-center justify-between px-3 py-1.5" style={{ background: 'var(--input-bg)', borderBottom: '1px solid var(--border-subtle)' }}>
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center gap-1.5"
          >
            <svg className={`w-3 h-3 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`} style={{ color: 'var(--fg-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-1">
            {isHtml && (
              <button
                ...
                className="p-1 rounded transition-colors" title="Előnézet"
            ...
            <button onClick={handleDownload} className="p-1 rounded transition-colors" title="Letöltés" style={{ color: 'var(--fg-muted)' }}>
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
              </svg>
            </button>
            <button onClick={handleCopy} className="p-1 rounded transition-colors" title="Másolás" style={{ color: copied ? 'var(--accent)' : 'var(--fg-muted)' }}>
              {copied ? (
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isExpanded && (
          <pre className="overflow-x-auto p-3 text-[13px] leading-relaxed" style={{ background: 'var(--surface-hover)', color: 'var(--fg)', maxHeight: '400px' }}><code>{code}</code></pre>
        )}
      </div>
      {showHtmlPreview && (
        <HtmlPreviewModal
